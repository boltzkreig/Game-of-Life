...
int main(int argc, char * argv[]) {
	...
	while (code.size() < (long unsigned int)(8 - len) ){code.push_back(0);}
	for( int i=0; i < len; i++){
		code.push_back((int)argv[1][i] - (int)'0');
	}


	if(argc > 2){r = strtod(argv[2], NULL);}
	if(argc > 3){delay = (int)(1'000'000/strtod(argv[3], NULL));}
   	if(argc > 4){strcpy(display, argv[4]);}
